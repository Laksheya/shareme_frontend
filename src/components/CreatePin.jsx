...
const CreatePin = () => {
  ...
  const uploadImage = async (e) => {
    ...
    if (!selectedFile) {
      return; // handle cases where no file is selected
    }

    if (type == 'image/png' || type == 'image/svg' || type == 'image/jpg' || type == 'image/jpeg' ||
  ...
  const savePin = async () => {
    if (title && about && destination && category && imageAsset?.imageId) {
      const doc = {
        title,
        about,
        destination,
